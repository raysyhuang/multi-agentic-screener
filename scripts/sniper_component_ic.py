...
⚠️ EXECUTION-CONFIG DEVIATION — read before quoting `trade_pnl_pct`.
The V3_* constants below are the FROZEN V3 research params (trail 1.0/0.5, zero
slippage). They are NOT the live execution config, which is trail 0.5/0.3 with
10bp/side and gap-through fills (see CLAUDE.md; `get_settings()` is the source
of truth). That is deliberate here — the IC targets are the RAW forward returns,
and `trade_pnl_pct` is a secondary sanity column only. It means:
  - The component ICs vs fwd_3/5/7/10d are unaffected and are the finding.
  - `trade_pnl_pct` levels are NOT comparable to any live or unified-engine
    number, and must not be quoted as a sniper expectancy.
Anyone extending this to make an expectancy claim must first route it through
`src/backtest/exit_engine.py` at settings-derived params. Hardcoded exit params
in a replay harness manufactured three fake live-vs-engine gaps in the 2026-07
MR reconciliation; this note exists so that does not happen a fourth time.
