...
1c. **A concentration rule — NOT YET REGISTERED, and until it is, no stream is decision-eligible.** The entry-day COUNT does not control concentration: 16 trades on one day plus 14 singletons is 15 days, which passes 1b, while that one day carries 53% of the estimate and the sample is worth about 3.3 balanced days. `effective_clusters` (Kish `1/Σw²`) and `max_cluster_share` are exported for this, and `decision_eligible` is **false for every stream** while no threshold exists — deferring a choice is only honest if the deferral fails closed. Candidate bar from the round-4 review: ≥ 20 distinct dates and ≥ 15 effective clusters. **This must be settled before any stream reaches n=30**, and it is free to settle now because no stream has an in-window trade yet. Setting it later, with a result visible, is a choice about that result.
...
> **Conditions 2 and 5 apply different evidentiary standards, and 5 is the weaker.** The normative statement, including both prohibitions, is at [Tier 2](#tier-2--the-promotion-bar-n--30) and is deliberately **not restated here** — this passage previously duplicated it near-identically, which is two places to drift and no rule for which wins.
