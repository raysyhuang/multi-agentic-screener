"""
Telegram notification module.

Sends an HTML-formatted daily summary to the configured Telegram chat
after each screener run. Includes market regime, signal count, and
per-ticker details with Finnhub headlines.
"""

import html
import logging
import ssl

import aiohttp
import certifi

from app.config import TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID
from app.paper_tracker import MOMENTUM_QUALITY_FLOOR, SKIP_BEARISH_REGIME

logger = logging.getLogger(__name__)

TELEGRAM_API = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
_ssl_ctx = ssl.create_default_context(cafile=certifi.where())


def _esc(text: str) -> str:
    """Escape HTML special characters for Telegram parse_mode=HTML."""
    if not text:
        return ""
    return html.escape(str(text), quote=False)


def _regime_dot(regime_str: str) -> str:
    """Map regime string to coloured circle emoji."""
    r = regime_str.lower() if regime_str else ""
    if "bull" in r:
        return "\U0001f7e2"
    if "bear" in r:
        return "\U0001f534"
    return "\U0001f7e1"


def _format_flow(sig: dict) -> str:
    """Format options flow as HTML-safe string."""
    sentiment = sig.get("options_sentiment")
    pcr = sig.get("put_call_ratio")
    if not sentiment or sentiment == "Neutral":
        if pcr is not None:
            return f"Flow: Neutral (P/C: {_esc(str(pcr))})"
        return "Flow: --"
    icon = "\U0001f402" if sentiment == "Bullish" else "\U0001f43b"
    pcr_str = f" (P/C: {_esc(str(pcr))})" if pcr is not None else ""
    return f"Flow: {icon} {_esc(sentiment)}{pcr_str}"


def _build_message(
    screener_result: dict,
    news_map: dict[str, list[dict]],
    reversion_result: dict | None = None,
) -> str:
    """
    Build a unified Telegram HTML message with both momentum
    and mean-reversion signals.
    """
    d = screener_result["date"]
    regime = screener_result["regime"]
    signals = screener_result["signals"]
    regime_str = regime.get("regime", "Unknown")

    rdot = _regime_dot(regime_str)

    # Header
    lines = [
        "<b>[Gemini STST] Daily Screener</b>",
        f"\U0001f4c5 {_esc(str(d))}  \u2022  {rdot} {_esc(regime_str)}",
        "",
    ]

    skip_mom = False
    if regime_str == "Bearish":
        lines.append("\u26a0\ufe0f Bearish Regime \u2014 exercise caution")
        if SKIP_BEARISH_REGIME:
            lines.append("\u26d4 Momentum trades SKIPPED (bearish regime filter)")
            skip_mom = True
        lines.append("")

    # --- Momentum Section ---
    tradeable = [s for s in signals if (s.get("quality_score") or 0) >= MOMENTUM_QUALITY_FLOOR]
    n_mom = len(signals)
    n_trade = len(tradeable) if not skip_mom else 0
    if n_trade < n_mom and not skip_mom:
        lines.append(f"\u2500\u2500 \U0001f4c8 Momentum Breakouts ({n_mom} signals, {n_trade} tradeable Q&gt;={MOMENTUM_QUALITY_FLOOR}) \u2500\u2500\u2500\u2500\u2500\u2500\n")
    else:
        lines.append(f"\u2500\u2500 \U0001f4c8 Momentum Breakouts ({n_mom}) \u2500\u2500\u2500\u2500\u2500\u2500\n")

    if not signals:
        lines.append("No momentum signals today.")
        lines.append("")
    else:
        for sig in signals:
            sym = sig["symbol"]
            price = sig["trigger_price"]
            rvol = sig["rvol_at_trigger"]
            atr = sig["atr_pct_at_trigger"]
            quality = sig.get("quality_score")
            confluence = sig.get("confluence", False)

            # Mark signals that won't be traded
            will_trade = (quality or 0) >= MOMENTUM_QUALITY_FLOOR and not skip_mom
            badge = "\u2b50 " if confluence else ""
            skip_icon = "" if will_trade else "\U0001f6ab "
            lines.append(f"{badge}{skip_icon}<b>{_esc(sym)}</b> \u2014 ${_esc(str(price))}")

            rsi_14 = sig.get("rsi_14")
            pct_52w = sig.get("pct_from_52w_high")

            flow_str = _format_flow(sig)
            detail_parts = [
                f"RVOL: {_esc(str(rvol))}",
                f"ATR: {_esc(str(atr))}%",
            ]
            if rsi_14 is not None:
                detail_parts.append(f"RSI: {_esc(str(rsi_14))}")
            if pct_52w is not None:
                detail_parts.append(f"52w: {_esc(str(pct_52w))}%")
            detail_parts.append(flow_str)
            if quality is not None:
                detail_parts.append(f"Q: {_esc(str(quality))}")
            lines.append("  " + " | ".join(detail_parts))

            if confluence:
                lines.append("  \u2b50 Confluence: recent reversion signal")

            articles = news_map.get(sym, [])
            for article in articles[:2]:
                headline = _esc(article.get("headline", ""))
                lines.append(f"  \u2022 {headline}")

            lines.append("")

    # --- Reversion Section ---
    rev_signals = reversion_result.get("signals", []) if reversion_result else []
    n_rev = len(rev_signals)
    lines.append(f"\u2500\u2500 \U0001f504 Oversold Reversions ({n_rev}) \u2500\u2500\u2500\u2500\u2500\u2500\n")

    if not rev_signals:
        lines.append("No oversold reversals today.")
    else:
        for sig in rev_signals:
            sym = sig["symbol"]
            price = sig["trigger_price"]
            rsi2 = sig["rsi2"]
            dd = sig["drawdown_3d_pct"]
            quality = sig.get("quality_score")
            confluence = sig.get("confluence", False)
            flow_str = _format_flow(sig)

            badge = "\u2b50 " if confluence else ""
            lines.append(f"{badge}<b>{_esc(sym)}</b> \u2014 ${_esc(str(price))}")

            detail_parts = [
                f"RSI(2): {_esc(str(rsi2))}",
                f"3d Drop: {_esc(str(dd))}%",
                flow_str,
            ]
            if quality is not None:
                detail_parts.append(f"Q: {_esc(str(quality))}")
            lines.append("  " + " | ".join(detail_parts))

            if confluence:
                lines.append("  \u2b50 Confluence: recent momentum signal")

            lines.append("")

    return "\n".join(lines)


async def send_telegram_alert(
    screener_result: dict,
    news_map: dict[str, list[dict]] | None = None,
    reversion_result: dict | None = None,
) -> bool:
    """
    Send the unified daily summary (momentum + reversion) to Telegram.

    Returns True on success, False on failure.
    """
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        logger.warning("Telegram credentials not configured — skipping alert")
        return False

    if news_map is None:
        news_map = {}

    message = _build_message(screener_result, news_map, reversion_result)

    payload = {
        "chat_id": TELEGRAM_CHAT_ID,
        "text": message,
        "parse_mode": "HTML",
        "disable_web_page_preview": True,
    }

    try:
        connector = aiohttp.TCPConnector(ssl=_ssl_ctx)
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.post(TELEGRAM_API, json=payload) as resp:
                if resp.status == 200:
                    logger.info("Telegram alert sent successfully")
                    return True
                else:
                    body = await resp.text()
                    logger.error("Telegram API error %d: %s", resp.status, body)
                    return False
    except Exception as e:
        logger.error("Failed to send Telegram alert: %s", e)
        return False
